...
contract Bora {
    ...
    event ReputationUpdated(address indexed developer, uint256 newReputation, uint256 timestamp);

    event EscrowDeposited(uint256 indexed opportunityId, uint256 amount, uint256 timestamp);

    event PlatformFeeUpdated(uint256 oldFee, uint256 newFee, uint256 timestamp);

    event OpportunityClosed(uint256 indexed opportunityId, uint256 refundAmount, uint256 timestamp);
    ...
    modifier onlyDeveloper() {
        if (!isDeveloper[msg.sender]) revert NotDeveloper();
        _;
    }

    modifier validOpportunity(uint256 _opportunityId) {
        if (_opportunityId >= _opportunityIdCounter) revert InvalidOpportunityId();
        if (!opportunities[_opportunityId].isActive) revert OpportunityNotActive();
        _;
    }
    ...
    function createDeveloperProfile(string memory _name, string[] memory _skills, string memory _bio)
        external
        whenNotPaused
    {
    ...
    function updateDeveloperProfile(string memory _name, string[] memory _skills, string memory _bio)
        external
        onlyDeveloper
        whenNotPaused
    {
    ...
    function createOpportunity(
        ...
        emit OpportunityCreated(newOpportunityId, msg.sender, _title, _budget, block.timestamp);
    }

    function depositEscrow(uint256 _opportunityId) external payable validOpportunity(_opportunityId) whenNotPaused {
    ...
    function closeOpportunity(uint256 _opportunityId) external nonReentrant {
        Opportunity storage opp = opportunities[_opportunityId];
        require(opp.creator == msg.sender, "Not the creator");
        ...
        emit OpportunityClosed(_opportunityId, refund, block.timestamp);
    }

    // ============ SWIPE FUNCTIONS ============

    function swipeOpportunity(uint256 _opportunityId, bool _isLike)
        external
        onlyDeveloper
        validOpportunity(_opportunityId)
        whenNotPaused
    {
        require(swipes[msg.sender][_opportunityId].timestamp == 0, "Already swiped this opportunity");

        Opportunity storage opp = opportunities[_opportunityId];
        require(opp.creator != msg.sender, "Cannot swipe own opportunity");

        swipes[msg.sender][_opportunityId] =
            Swipe({user: msg.sender, opportunityId: _opportunityId, isLike: _isLike, timestamp: block.timestamp});
    ...
    function acceptMatch(uint256 _opportunityId, address _developer)
        external
        validOpportunity(_opportunityId)
        whenNotPaused
    {
        ...
        emit MatchCreated(newMatchId, _developer, _opportunityId, msg.sender, block.timestamp);
    }

    function completeJob(uint256 _matchId, uint256 _reputationIncrease) external whenNotPaused nonReentrant {
        ...
        uint256 payment = opp.escrowBalance;
        uint256 developerPayment = 0;

        if (payment > 0) {
            // Calculate platform fee
            uint256 fee = (payment * platformFee) / 10000;
            developerPayment = payment - fee;

            opp.escrowBalance = 0;

            // Transfer to developer
            (bool success,) = payable(matchData.developer).call{value: developerPayment}("");
            if (!success) revert TransferFailed();
        ...
        emit JobCompleted(_matchId, matchData.developer, developerPayment, dev.reputation, block.timestamp);

        emit ReputationUpdated(matchData.developer, dev.reputation, block.timestamp);
    }

    // ============ VIEW FUNCTIONS ============

    function getDeveloper(address _developer) external view returns (Developer memory) {
    ...
    function getUserMatches(address _user) external view returns (Match[] memory) {
    ...
    function getOpportunitySwipes(uint256 _opportunityId) external view returns (address[] memory) {
        require(opportunities[_opportunityId].creator == msg.sender, "Not the creator");
        return opportunitySwipes[_opportunityId];
    }

    function getSwipe(address _user, uint256 _opportunityId) external view returns (Swipe memory) {
